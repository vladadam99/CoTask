      ...
      <div className="space-y-2">
        ...
        <Link to={liveMatch[0]}>
          <Button size="sm" className="h-8 gap-1.5 text-xs">
            <Video className="w-3.5 h-3.5" /> Open live session
          </Button>
        </Link>
      </div>
    );
                ...
                <div key={m.id} className={`flex ${m.sender_email === user?.email ? 'justify-end' : 'justify-start'}`}>
                  {m.message_type === 'system' ? (
                    <div className="w-full flex justify-center">
                      <div className="max-w-sm text-center px-3 py-2 rounded-xl bg-secondary/60 border border-border text-xs text-muted-foreground">{renderMessageContent(m)}</div>
                    </div>
                  ) : (
