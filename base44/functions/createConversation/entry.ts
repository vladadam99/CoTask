...
Deno.serve(async (req) => {
  try {
    ...
    if (booking.client_email !== user.email && booking.avatar_email !== user.email && user.role !== 'admin') {
    ...
    await base44.asServiceRole.entities.Notification.create({
      user_email: isClient ? booking.avatar_email : booking.client_email,
      title: `New conversation about booking`,
      message: `${booking.category} · ${booking.duration_minutes || 60} min`,
      type: 'booking_request',
      link: isClient ? `/AvatarBookingDetail?id=${bookingId}` : `/UserBookingDetail?id=${bookingId}`,
